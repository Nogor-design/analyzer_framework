...
**Episode Boundaries:** Single trading session (RTH only). Episodes start at 08:30 MT and end at 15:00 MT. This gives ~78 bars per episode at 5-minute resolution.

<!-- @ANNOTATION [2026-04-07] — LIKELY ERROR in session times: The codebase uses
...
**Risks:** Strategy recommendations may be sparse if regime coverage is uneven.

**Success Criteria:** System produces a concrete, actionable recommendation (strategy family + parameters) for each trading session. Human operator uses it for at least 10 sessions and records qualitative feedback.
...
7. Build full gateway + risk engine (Phase 4)
8. Build RL environment (Phase 5)
9. Enable live mode (Phase 6, gated)
