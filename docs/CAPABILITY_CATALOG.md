...
| `large-candle` | 7796 | `analysis/large_candle_excursion` — Adaptive windows (`run_adaptive_large_candle_windows`, strategy-parity fixed-bracket walk-forward) plus Sweep → Findings → Legacy regime → Excursion stability | Adaptive engine added 2026-07-25. Keeps parameter lanes isolated, compares continuation/reversion from completed prior outcomes, and freezes weekly time-window maps. Legacy `stable_edge` labels describe excursion-touch persistence, not stop-aware P&L. |
| `ma-anchor` | 7797 | `analysis/ma_structure` — MA anchor *interaction* (approach/cross/ride segments, anchor-relative TP/SL): Explore (`build_anchors_table`/`detect_segments`/`compute_path_metrics`/`build_summary_by_anchor`) · Discover (`score_tp_sl_candidates` + by-regime, net-of-cost) · Prove (`build_validation_folds` IS/OOS + cross-instrument pooled t). Distinct from `ma-cross` (crossover *entries*). | Shipped & verified 2026-06-26 (wrappers-only; `smoke_test.py` 3/3 on NQ 06-26: 1,478 segments, TP/SL scored, panel proof. Honest read: naive anchor-relative rule is cost-walled — pooled t≈−45, 0/4 instruments profitable; value is structural excursion/regime context, not a signal). |
| `prediction` | 7798 | `prediction/` (daily + horizon) — own operational app, NOT kit-shaped: Daily (`predict_next_day`/`measure_and_learn`/`compute_agent_stats`) · Horizon (`run_horizon_backtest` + `HorizonBatchRunner`, stacking ensemble) · Reports (`build_full_report`, six reports) · Deploy (`HorizonConfig` tradable-zone verdict over `prediction.yaml`) | Shipped & verified 2026-06-26 (Phases 0–7; wrappers-only, no-cost default paths, env-only API key; `smoke_test.py` 47/47 no key/cost). Honest read: baseline agents ~0.39–0.46 dir-acc at coarse strides — an instrument panel for tradability-after-costs, not a signal generator. |
