# src/ta_foundation/reports/html/sections/exit_policy_simulation2.py
...
def _nan_safe_series(v: pd.Series) -> pd.Series:
    s = pd.to_numeric(v, errors="coerce")
    return s.dropna()


def _p_quantile(s: pd.Series, q: float) -> float:
    s2 = _nan_safe_series(s)
    if s2.empty:
        return 0.0
    return float(s2.quantile(q))


def _cvar_lower(s: pd.Series, q: float = 0.05) -> float:
    """Conditional Value at Risk (lower tail): mean of values <= q-quantile."""
    s2 = _nan_safe_series(s)
    if s2.empty:
        return 0.0
    thresh = float(s2.quantile(q))
    tail = s2[s2 <= thresh]
    if tail.empty:
        return thresh
...
def _max_drawdown_ticks(pnl_ticks: pd.Series) -> float:
    """Max drawdown of cumulative pnl (in ticks). Returns a positive number (ticks)."""
    s = _nan_safe_series(pnl_ticks)
    if s.empty:
        return 0.0
    eq = s.cumsum()
    peak = eq.cummax()
...
def _max_consecutive_losses(pnl_ticks: pd.Series) -> tuple[int, float]:
    """Return (max_loss_streak_count, max_loss_streak_ticks_positive)."""
    s = _nan_safe_series(pnl_ticks)
    if s.empty:
        return 0, 0.0
    max_streak = 0
    ...
    for v in s.tolist():
        if v < 0:
            ...
            cur_streak = 0
            cur_loss = 0.0
    return int(max_streak), float(max_streak_loss)
...
def render_exit_policy_simulation2(ctx: Dict[str, Any]) -> str:
    """
    ...
    # Collect per-run/per-policy stats for cross-run ranking
    all_run_policy_stats: List[Dict[str, Any]] = []
    bots_policy_net: List[Dict[str, Any]] = []
    ...
    for run_id, n_trades in run_rows:
        ...
        # Sort trades consistently for drawdown / streak metrics
        sort_col = _find_col(sim, ["trade_idx", "trade_index", "idx", "entry_dt", "entry_time", "Entry time"])
        if sort_col:
            sim = sim.sort_values(sort_col, kind="mergesort")
        ...
        # Downside metrics per policy (per-run)
        for pol, g in sim.groupby("policy"):
            pnl = g["pnl_ticks"]
            max_dd = _max_drawdown_ticks(pnl)
            worst_trade = float(_nan_safe_series(pnl).min()) if not _nan_safe_series(pnl).empty else 0.0
            p05 = _p_quantile(pnl, 0.05)
            cvar05 = _cvar_lower(pnl, 0.05)
            loss_streak_n, loss_streak_ticks = _max_consecutive_losses(pnl)
            all_run_policy_stats.append(
                {
                    "run_id": run_id,
                    "policy": str(pol),
                    "trades": int(len(_nan_safe_series(pnl))),
                    "net_ticks": float(_nan_safe_series(pnl).sum()) if not _nan_safe_series(pnl).empty else 0.0,
                    "avg_ticks": float(_nan_safe_series(pnl).mean()) if not _nan_safe_series(pnl).empty else 0.0,
                    "win_rate": float((_nan_safe_series(pnl) > 0).mean()) if not _nan_safe_series(pnl).empty else 0.0,
                    "p05": float(p05),
                    "cvar05": float(cvar05),
                    "worst_trade": float(worst_trade),
                    "max_dd_ticks": float(max_dd),
                    "max_loss_streak": int(loss_streak_n),
                    "max_loss_streak_ticks": float(loss_streak_ticks),
                }
            )

        # For bots x policies matrix
        row = {"run_id": run_id}
        for _, r in agg.iterrows():
            row[str(r["policy"])] = float(r["net_ticks"])
        bots_policy_net.append(row)
    ...
    # --- Cross-run downside-focused ranking ---
    if all_run_policy_stats:
        d = pd.DataFrame(all_run_policy_stats)

        # Aggregate across all displayed runs by pooling trades per policy
        # (Drawdown is reported as: worst_run_max_dd (max across runs) + avg_run_max_dd.)
        per_run = d.copy()

        overall = (
            d.groupby("policy", as_index=False)
            .agg(
                runs=("run_id", "nunique"),
                trades=("trades", "sum"),
                net_ticks=("net_ticks", "sum"),
                avg_ticks=("avg_ticks", "mean"),
                win_rate=("win_rate", "mean"),
                p05=("p05", "mean"),
                cvar05=("cvar05", "mean"),
                worst_trade=("worst_trade", "min"),
                worst_run_max_dd=("max_dd_ticks", "max"),
                avg_run_max_dd=("max_dd_ticks", "mean"),
                worst_run_loss_streak=("max_loss_streak", "max"),
                worst_run_loss_streak_ticks=("max_loss_streak_ticks", "max"),
            )
        )

        # Constraints (optional)
        dd_limit = opts.get("dd_limit_ticks", None)
        p05_min = opts.get("p05_min_ticks", None)
        cvar05_min = opts.get("cvar05_min_ticks", None)
        worst_trade_min = opts.get("worst_trade_min_ticks", None)
        loss_streak_max = opts.get("max_loss_streak_max", None)
        ...
        dd_limit = _to_f(dd_limit)
        p05_min = _to_f(p05_min)
        cvar05_min = _to_f(cvar05_min)
        worst_trade_min = _to_f(worst_trade_min)
        loss_streak_max = int(loss_streak_max) if str(loss_streak_max).strip().isdigit() else None

        feasible = overall.copy()
        if dd_limit is not None:
            feasible = feasible[feasible["worst_run_max_dd"] <= dd_limit]
        if p05_min is not None:
            feasible = feasible[feasible["p05"] >= p05_min]
        if cvar05_min is not None:
            feasible = feasible[feasible["cvar05"] >= cvar05_min]
        if worst_trade_min is not None:
            feasible = feasible[feasible["worst_trade"] >= worst_trade_min]
        if loss_streak_max is not None:
            feasible = feasible[feasible["worst_run_loss_streak"] <= loss_streak_max]

        # Ranking rule (downside-first):
        # 1) Filter to feasible (if constraints provided)
        # 2) Sort by: net_ticks desc, worst_run_max_dd asc, cvar05 desc
        rank_base = feasible if len(feasible) else overall
        rank_base = rank_base.sort_values(
            ["net_ticks", "worst_run_max_dd", "cvar05"], ascending=[False, True, False]
        ).reset_index(drop=True)

        html.append("<div class='tf-exit-card'>")
        html.append("<div class='tf-exit-title'>Downside-focused ranking (all displayed runs)</div>")

        # Show constraints used
        cons = []
        if dd_limit is not None:
            cons.append(f"worst_run_max_dd ≤ {dd_limit:.2f} ticks")
        if p05_min is not None:
            cons.append(f"p05 ≥ {p05_min:.2f} ticks")
        if cvar05_min is not None:
            cons.append(f"cvar05 ≥ {cvar05_min:.2f} ticks")
        if worst_trade_min is not None:
            cons.append(f"worst_trade ≥ {worst_trade_min:.2f} ticks")
        if loss_streak_max is not None:
            cons.append(f"max_loss_streak ≤ {loss_streak_max}")
        if cons:
            html.append(f"<div class='tf-exit-sub'>Constraints: <code>{_h(' ; '.join(cons))}</code></div>")
        else:
            html.append(
                "<div class='tf-exit-sub'>Sorting: net_ticks ↓, worst_run_max_dd ↑, cvar05 ↓ (less negative is better). Add constraints via options: <code>dd_limit_ticks</code>, <code>p05_min_ticks</code>, <code>cvar05_min_ticks</code>, <code>worst_trade_min_ticks</code>, <code>max_loss_streak_max</code>.</div>"
            )
        ...
        cols = [
            ...
            "worst_run_max_dd",
            "avg_run_max_dd",
            "worst_run_loss_streak",
            "worst_run_loss_streak_ticks",
        ]
        show = rank_base[cols].copy()
        html.append(_render_table(show.round(2), max_rows=30))

        # Chart: net ticks vs worst_run_max_dd (scatter)
        try:
            fig = plt.figure(figsize=(8.5, 3.0))
            ax = fig.add_subplot(111)
            ax.scatter(rank_base["worst_run_max_dd"], rank_base["net_ticks"])
            for _, r in rank_base.iterrows():
                ax.annotate(str(r["policy"]), (r["worst_run_max_dd"], r["net_ticks"]), fontsize=7)
            ax.set_xlabel("Worst-run Max DD (ticks)")
            ax.set_ylabel("Net ticks (sum across runs)")
            ax.set_title("Policy trade-off: Net ticks vs Worst-run drawdown")
            fig.tight_layout()
            uri = fig_to_base64_png(fig)
            plt.close(fig)
            html.append(f"<img class='tf-exit-img' src='{uri}' alt='net vs drawdown scatter' />")
        except Exception:
            pass

        # Bots x policies net ticks matrix
        if bots_policy_net:
            mx = pd.DataFrame(bots_policy_net).fillna(0.0)
            # order columns: run_id first, then by overall ranking order
            pol_order = rank_base["policy"].tolist()
            cols2 = ["run_id"] + [c for c in pol_order if c in mx.columns]
            other_cols = [c for c in mx.columns if c not in cols2]
            mx = mx[cols2 + other_cols]
            html.append("<h4 style='margin:10px 0 6px 0;'>Bots × Policies (net ticks)</h4>")
            html.append(_render_table(mx.round(1), max_rows=50))
