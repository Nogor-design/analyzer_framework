...
def _intraday_trailing_breach_prob_and_time(
    *,
    ...
    """
    breach_steps: first step index (1-based) when breached, or NaN.
    expected_time_to_breach: mean breach_steps conditional on breach.
    """
...
def build_markov_regime_model(
    *,
    ...
    daily_regimes: sequence indexed by day order
    """
...
def _compute_daily_vol_metric(
    *,
    ...
    """
    Returns per day_id:
      day_id, day_regime, day_vol (float)
    day_vol priority:
      - median(vol_metric) if present/non-null
      - else abs(sum(pnl_usd)) as fallback
    """
    d = events.copy()
...
def _assign_vol_buckets(daily: pd.DataFrame, *, q: float = 0.5) -> pd.DataFrame:
    """
    Simple 2-bucket split: low/high using median (or provided quantile).
    """
    d = daily.copy()
...
def apply_microstructure_stress(
    *,
    ...
    - slippage/spread widening: subtract (slippage + spread_widen) ticks per filled trade.
      (This is conservative; you can refine later with directionality / half-spread logic.)
    """
    ...
    if cost > 0:
        pnl = pnl - cost  # ticks cost per filled trade (conservative)
    return pnl
...
def apply_trade_intake_model(
    *,
    ...
    """
    Deterministic down-selection of raw events into executable trades to avoid signal-overtrading bias.

    Expected columns in events_df: dt, day_id. Other columns are preserved.
    intake knobs (all optional):
      ...
      random_k: int
      cooldown_scope: 'global' (default) | 'per_entity' (only meaningful when events include entity_id)
    Returns:
      (events_exec_df, intake_diagnostics)
    """
    ...
    if "dt" not in events_df.columns or "day_id" not in events_df.columns:
        # fail open (do not drop trades) but report
    ...
    for day_id, day_df in d.groupby("day_id", sort=False):
        ...
        if selection == "random_k":
            ...
            # take_first: keep time order
            cand = cand

        # cooldown stage (sequential)
        if cooldown_minutes > 0:
            ...
            for idx, row in cand.iterrows():
                ...
                if pd.isna(dt):
                    # if dt bad, keep but doesn't advance cooldown
    ...
    diag = {
        ...
        "per_day_counts_out": {str(k): int(v) for k, v in list(per_day_counts.items())[:50]},  # cap size
    }
    return out, diag
...
def _build_day_blocks_for_entity(
    *,
    ...
    """
    Returns dict:
      blocks[regime][vol_bucket] = list of day blocks, each block is pnl_usd array in intraday order.
    """
...
def _sample_path_regime_aware(
    *,
    ...
    blocks: Dict[str, Dict[str, List[np.ndarray]]],
    n_days: int,
    vol_mode: str = "bucketed",  # "bucketed" or "any"
    forced_vol_bucket: Optional[str] = None,
) -> List[np.ndarray]:
    """
    Samples a list of day pnl_usd arrays, respecting Markov transitions.
    """
    regimes = model.regimes
    k = len(regimes)

    # start state ~ stationary-ish: use empirical row sums of P as weights fallback uniform
    ...
    for _ in range(int(n_days)):
        ...
        if forced_vol_bucket is not None:
            ...
            if not pool:
                # fallback to any vol bucket under this regime
                pool = [b for vv in reg_blocks.values() for b in vv]
        elif vol_mode == "bucketed":
            # choose vol bucket available under regime weighted by count
        ...
        if not pool:
            # ultimate fallback: any block across all regimes/vol
...
def _simulate_intraday_trailing_dd(
    *,
    ...
    """
    Walk intraday events (trade-level) and trigger breach at first time peak-eq > trailing_dd.
    Returns:
      breached (bool), breach_step (1-based int, or 0), dd_max (float)
    """
    eq = 0.0
    peak = 0.0
    dd_max = 0.0
...
def _intraday_dd_from_open(day_pnls: np.ndarray) -> float:
    eq = np.cumsum(day_pnls)           # equity path within day starting at 0
    return float(np.min(eq))           # most negative dip from open baseline
...
def _simulate_prop_path(
    *,
    ...
    """
    Apex-style evaluation model with:
      - Intraday trailing drawdown tracking peak *unrealized* balance.
      - Trailing stop "locks" once peak >= start + trailing_dd + buffer, at floor = start + buffer.
      - Daily loss computed intraday from day open equity.
      - Profit target: pass if equity >= start + profit_target_usd.

    Returns JSON-safe summary dict:
      status: "pass" | "fail" | "survive"
      fail_reason: "daily_loss" | "trailing_dd" | None
      days_to_event: int (1-based within sampled window) or 0
      steps_to_trailing_breach: int (1-based trade step) or 0
      dd_max_usd: float (max peak-to-trough drawdown over path)
    ...
    # trailing floor starts at liquidation threshold (start - trailing_dd)
    ...
    for di, day in enumerate(day_blocks, start=1):
        ...
        if dd_mode == "session_reset" and not locked:
            # reset peak tracking to day open; do NOT reduce floor below liquidation threshold
        ...
        for x in day_arr:
            ...
            # profit target can be hit intraday
            ...
            # trailing breach (intraday, includes unrealized peaks)
...
def run_prop_monte_carlo(
    *,
    ...
    """
    Backwards-compatible baseline MC summary (kept for existing sections).
    Expects events_df schema produced by engine.py:
    ...
    # Optional trade intake model (compute-only). If enabled, down-select raw events into executable trades.
    ...
    # Preserve backwards compatibility: use executable events for simulation if enabled; otherwise df_exec == df.
    df = df_exec
    ...
    for (etype, eid, horizon), d2 in df.groupby(["entity_type", "entity_id", "horizon"], sort=False):
        ...
        #     pnl = np.concatenate(sampled_days) if sampled_days else np.array([], dtype=float)
        #
        #     # daily loss breach on sampled day totals
        #     sampled_day_totals = [float(np.sum(day_blocks[int(j)])) for j in picks]
        #     if any(t < -daily_loss_usd for t in sampled_day_totals):
        #         daily_breach += 1
        #
        #     # trailing DD (intraday) on concatenated trades
        #     breached, _, ddmax = _simulate_intraday_trailing_dd(day_blocks=sampled_days, trailing_dd_usd=trailing_dd_usd)
        #     if breached:
        #         trailing_breach += 1
        #     dds[i] = ddmax
        ...
        dd_mode = _safe_str(mc_options.get("dd_mode", "global_peak"), "global_peak")  # global_peak|session_reset


        # Apex-style equity target + trailing lock (Safety Net) model options
...
def run_prop_monte_carlo_regime(
    *,
    ...
    """
    Regime-aware + volatility-conditioned + intraday trailing DD Monte Carlo.

    Required output artifact:
      mc_regime_summary_df with columns:
        entity_id
        horizon
        regime_model
        vol_bucket
    ...
    # evaluation window length (critical; otherwise daily breach -> ~1 for long histories)
    dd_mode = _safe_str(mc_options.get("dd_mode", "global_peak"), "global_peak")  # global_peak|session_reset
    ...
    for (eid, horizon), e2 in df.groupby(["entity_id", "horizon"], sort=False):
        ...
        for vol_bucket in all_vol_buckets:
            ...
            # deterministic per (entity,horizon,vol_bucket)
        ...
        # Pre-sample (regime-aware) day blocks once per path (deterministic)
        ...
        # Convert sampled USD pnl blocks -> ticks (so we can apply tick costs), preserving path order
        base_samples_ticks: List[np.ndarray] = []
        for day_blocks in base_samples:
            if not day_blocks:
                base_samples_ticks.append(np.array([], dtype=float))
            else:
                pnl_usd = np.concatenate(day_blocks) if len(day_blocks) else np.array([], dtype=float)
                pnl_ticks = pnl_usd / float(tick_value) if tick_value else pnl_usd * 0.0
                base_samples_ticks.append(pnl_ticks.astype(float))
