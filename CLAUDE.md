...
**This project is part of a multi-repo ecosystem. Before proposing ANY new capability, read `docs/CAPABILITY_CATALOG.md` (load-first router) and `docs/reference/EXTERNAL_PROJECTS_MAP.md`** — five external sibling repos (`D:\local-deep-research`, `D:\NinjatraderDocScrapper`, `D:\NinjaAccountManager`, `D:\DailyAnalysis`, `D:\agentic-engine`) already own large parts of the research→`.cs`→account→execution→prediction pipeline. Most "new" ideas already exist internally or in a sibling repo; the recurring failure mode here is rebuilding what exists. `docs/reference/COMPLETE_SYSTEM_MAP.md` maps internal capabilities but is being re-audited (2026-06) — verify claims against code before trusting them.
